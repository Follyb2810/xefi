        ...
        <Container maxW={"1200px"} p={5} my={5}>
            <SimpleGrid columns={2} spacing={6}>
                <Stack spacing={"20px"}>
                    ...
                    <Box>
                        <Text fontWeight={"bold"} color="brand.700">Description:</Text>
                        <Text color="brand.700">{nft.metadata.description}</Text>
                    </Box>
                    <Box>
                        <Text fontWeight={"bold"} color="brand.700">Traits:</Text>
                        <SimpleGrid columns={2} spacing={4}>
                        {Object.entries(nft?.metadata?.attributes || {}).map(
                        ([key, value]) => (
                            <Flex key={key} direction={"column"} alignItems={"center"} justifyContent={"center"} borderWidth={1} p={"8px"} borderRadius={"4px"}>
                                <Text fontSize={"small"} color="brand.700">{value.trait_type}</Text>
                                <Text fontSize={"small"} fontWeight={"bold"} color="brand.700">{value.value}</Text>
                            </Flex>
                        )
                        )}
                        </SimpleGrid>
                ...
                <Stack spacing={"20px"}>
                    {contractMetadata && (
                        <Flex alignItems={"center"}>
                            ...
                            <Text fontWeight={"bold"} color="brand.700">{contractMetadata.name}</Text>
                        </Flex>
                    )}
                    <Box mx={2.5}>
                        ...
                        >
                            <Flex direction={"row"} alignItems={"center"}>
                                ...
                                <Text fontSize={"small"} color="brand.700">{nft.owner.slice(0,6)}...{nft.owner.slice(-4)}</Text>
                            </Flex>
                        </Link>
                    </Box>
